...
class PatientRegisterActivity : AppCompatActivity() {
    private val TAG: String= PatientRegisterActivity::class.java.simpleName
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.fragment_patient_register_)

        companyimage=findViewById(R.id.companyImg)
        companyphotoeditbtn=findViewById(R.id.imageButton2)
        ...
        bitmapUtility= BitmapUtility(this)
    ...
    private fun askAppointment(){
        ...
        if (Doctorname.isNullOrEmpty()){
            ...
            list.add(PatientRegisterTable(Doctorname,Doctornumber,logo,Doctoremail,Special,Pass))
...
//            list = patientRegisterDao.getall() as MutableList<DoctorRegisterTable>
            intent = Intent(this@PatientRegisterActivity,PacientLoginActivity::class.java)
...
//                Log.e("TAG", "select event for photo=$resultCode")
            ...
            1 -> {
                if (resultCode == Activity.RESULT_OK && requestCode == 1) {
                    ...
                    if (data != null) {
                        try {
                            bm = MediaStore.Images.Media.getBitmap(this.contentResolver, data.data)
...
//                            patientRegisterDao.insert(list)
                        ...
                        Toast.makeText(this,"Only csv file allowed",Toast.LENGTH_LONG).show()
                    }
                }catch (e:Exception){
                    Log.e(TAG,e.localizedMessage)
                }
